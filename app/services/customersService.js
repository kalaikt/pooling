...
app.service('customersService', function () {
	...
	this.authenticate = function(email, password, $scope, $http, $location){
		var user;
		$http.post("http://localhost/pooling/webservice/?s=login", {"email":email, "password":password})
		.success(function(data, status, headers, config) {
			 if(data.id) {$location.path('/register');}else{$scope.error_message = 'Invalid email or password';$scope.hide=false;}
		}).error(function(data, status, headers, config) {
			 $location.path('/register');
		});
	};
	
	this.getAds = function($scope, $http, $location){
		$http.post("http://localhost/pooling/webservice/?s=ads")
		.success(function(data, status, headers, config) {
			 $scope.ads = data;
		}).error(function(data, status, headers, config) {
			// $location.path('/register');
